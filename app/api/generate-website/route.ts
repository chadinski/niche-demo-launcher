...
export async function POST(request: Request) {
  ...
  if (wantsStream) return new Response(new ReadableStream({ async start(controller) {
    ...
    catch (error) { const errorCode = error instanceof ApiError ? error.code : "GENERATION_FAILED"; serverLog("error", "generation.pipeline_failed", { generationId: id, requestId, code: errorCode, reason: error instanceof Error ? error.message.slice(0, 180) : "unknown" }); await completeUsage(reservation!, usageStatusForPipelineOutcome(false), { errorCode }); await finishGenerationJob(user, id, "failed", errorCode); send({ type: "error", generationId: id, error: "Website generation could not complete. Your previous saved version is unchanged.", code: errorCode }); }
  ...
  catch (error) { const errorCode = error instanceof ApiError ? error.code : "GENERATION_FAILED"; serverLog("error", "generation.pipeline_failed", { generationId: id, requestId, code: errorCode, reason: error instanceof Error ? error.message.slice(0, 180) : "unknown" }); await completeUsage(reservation, usageStatusForPipelineOutcome(false), { errorCode }); await finishGenerationJob(user, id, "failed", errorCode); const safe = userSafeError(error, "Website generation could not complete. Your previous saved version is unchanged."); return NextResponse.json({ generationId: id, requestId, ...safe.body }, { status: safe.status, headers }); }
}
